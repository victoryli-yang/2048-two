...
// 更新界面
const updateBoardView = function() {
    updateScore(score)
    $('.number-cell').remove()
        for (var i = 0; i < 4; i++) {
            for (var j = 0; j < 4; j++) {
...
// 随机数
const generateOneNumber = function() {
    if (noSpace(board)) {
        return false
    }
    //随机位置
...
// 判断
var judgmentToLeftRight = function(i, k, j, board) {
...
var judgmentToUpDown = function(i, k, j, board) {
...
var moveLeft = function() {
    ...
    for (var i = 0; i < 4; i++) {
        for (var j = 1; j < 4; j++) {
            if (board[i][j] !== 0) {
                for (var k = 0; k < j; k++) {
                    judgmentToLeftRight(i, k, j, board)
                }
            }
        }
    }
